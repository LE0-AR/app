...
if (isset($_GET['id'])) {
    ...
    if ($connect->query($sql_producto) === TRUE) {
        echo "<script>alert('Producto eliminado correctamente'); window.location.href = '../../Views/user/ProductUser.php';</script>";
    } else {
        echo "<script>alert('Error al eliminar el producto'); window.location.href = '../../Views/user/ProductUser.php';</script>";
    }

    $connect->close(); // Cerrar conexión
} else {
    echo "<script>alert('ID de producto no proporcionado'); window.location.href = '../../Views/user/ProductUser.php';</script>";
}
?>